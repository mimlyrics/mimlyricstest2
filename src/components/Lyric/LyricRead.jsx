  ...
  const searchLyricsF = async () => {
   ...
   useEffect(() => {
    const similarLyrics = async () => {
        try {
            const res = await axios.put(`${RECOMMENDED_LYRIC_URL}/${category}`, {artistName},
             {headers: {withCredentials: true}});
  ...
  return (
    <section className={ isActiveModalNavbar ? "relative -z-50 opacity-60" : "md:absolute md:top-16 md:ml-[20%] mx-1 md:mx-3"}>
      <div className=' my-2  '>
        ...
        <div className=' md:w-[50vw] bg-fuchsia-50 text-gray-950 '>
          {text.map(t=> {
            return <div 
        ...
        <div className='  py-1 text-gray-900 '>
          ...
          <p className=' mx-2 md:w-[40vw]'>{lyric.description}</p>       
        </div>
      </div>
      ...
      <div className='mx-2 rounded-md '>
          { rLyrics ? rLyrics.map((lyric,i) => {
            return (<div className=' ' key={lyric._id}>
              <Link to={`/post/lyric/read?lyricId=${lyric._id}`} 
               className=' '>
               <div className=' px-2 bg-fuchsia-50 hover:bg-fuchsia-100 py-2 flex space-x-3 space-y-0'>
      ...
      <div className='my-2 md:mt-1 grid max-w-[700px] grid-cols-3 px-5 lg:grid-cols-3 gap-2 lg:gap-5'>          
