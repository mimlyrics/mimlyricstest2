...
const EditorAlbum = () => {
  ...
  return (
    <section className=" md:absolute md:top-2 md:w-[50%] md:ml-[27%] my-1 py-1">
      <div className={isChosen ? "" : ""}>
        {/** Get videos to post */}
            ...
            >
                ...
                return (<option className=" italic text-center rounded-full shadow bg-zinc-300 h-20 "  key={categoryx._id} value={categoryx.name}>{categoryx.name}</option>)
                }) : null}
            </select>
            </div>
        </div>
        {isChosen ? 
         <div className="mx-1">
            ...
            <div className="my-3">
                <Link  to= "/post/editor/album1"  className=" w-11 h-4 p-2 border shadow rounded-lg bg-blue-100 hover:bg-blue-300 
                            ...
                            <div key={album._id} className="border-b-2 text-[17px] font-medium py-3">
                                ...
                                {album.lyric.map((l,i) => {
                                    return (<div key={l._id} className=" mb-2 ">
                                            {l.title ? <p className="flex text-gray-800">Title: <p className="ml-2 f text-blue-900">{i}. {l.title.substring(0,40)}...</p></p> : null}
                                ...
                                <div className="mt-3">
                                    <button><Link onClick={e=> !album._id ? e.preventDefault(): null}  to= {`/post/editor/album/edit?albumId=${album._id}`}  className=" p-3 border shadow rounded-lg bg-green-300" >Edit</Link> </button>
                                    <button onClick={()=>deleteAlbum(album._id)} className=" ml-4 p-3 border shadow rounded-lg bg-red-400" >Delete</button>
                                </div>                        
                            </div> 
                        )
                      }) 
